...
bool bfs(int **ptr_cols, int n,int* buffer,int vertex = 0);
...
//	инициализация матрицы смежности
	...
	int vertex = 0;
	bool isCicle = bfs(ptr_cols, n, buffer,vertex);
	if(isCicle){
		std::cout << "В графе есть цикл!";
	}else {
		std::cout << "В графе нет цикла!";
...
bool bfs(int **ptr_cols, int n, int* buffer, int vertex) {
	bool isCicle = false;
	int lastIndex = 0;
	buffer[vertex] = 1;
	for (int j = 0; j < n; j++) {
		for (int i = 0; i < n; i++) {
			if (ptr_cols[vertex][i] == 1 ) {
